...
const tasks = loadTasksFromLocalStorage(); // Load tasks from local storage
...
function loadDashboardPage() {
    ...
    if (viewContainer) {
        ...
        // Categorize tasks
        const expiredTasks = tasks.filter((task) => !task.completed && new Date(task.date) < currentDate);
...
function updateTaskCounts() {
    const currentDate = new Date();
    const expiredTasks = tasks.filter((task) => !task.completed && new Date(task.date) < currentDate);
    const notDoneTasks = tasks.filter((task) => !task.completed && new Date(task.date) >= currentDate);
    const doneTasks = tasks.filter((task) => task.completed && new Date(task.date) >= currentDate);
